...
if platform.system() == "Windows":
    ADC_GET_DATA_HANDLE = WINFUNCTYPE(
        c_int, c_int, c_void_p, c_int
    )  # First argument is return type
else:
    ADC_GET_DATA_HANDLE = CFUNCTYPE(
        c_int, c_int, c_void_p, c_int
    )  # First argument is return type
...
def ADC_StartContinueRead(DevHandle, Channel, SampleRateHz, FrameSize, pGetDataHandle):
    return USB2XXXLib.ADC_StartContinueRead(
        DevHandle, Channel, SampleRateHz, FrameSize, pGetDataHandle
    )
