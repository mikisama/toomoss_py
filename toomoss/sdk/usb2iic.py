...
def IIC_WriteBytes(DevHandle, IICIndex, SlaveAddr, pWriteData, WriteLen, TimeOutMs):
    return USB2XXXLib.IIC_WriteBytes(
        DevHandle, IICIndex, SlaveAddr, pWriteData, WriteLen, TimeOutMs
    )


# 主机模式读数据
def IIC_ReadBytes(DevHandle, IICIndex, SlaveAddr, pReadData, ReadLen, TimeOutMs):
    return USB2XXXLib.IIC_ReadBytes(
        DevHandle, IICIndex, SlaveAddr, pReadData, ReadLen, TimeOutMs
    )


# 主机模式写读数据，写数据和读数据之间有重复启动信号，无停止信号
def IIC_WriteReadBytes(
    DevHandle, IICIndex, SlaveAddr, pWriteData, WriteLen, pReadData, ReadLen, TimeOutMs
):
    return USB2XXXLib.IIC_WriteReadBytes(
        DevHandle,
        IICIndex,
        SlaveAddr,
        pWriteData,
...
def IIC_SlaveWriteBytes(DevHandle, IICIndex, pWriteData, WriteLen, TimeOutMs):
    return USB2XXXLib.IIC_SlaveWriteBytes(
        DevHandle, IICIndex, pWriteData, WriteLen, TimeOutMs
    )
