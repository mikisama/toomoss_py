...
CANFD_DIAGNOSTIC_FLAG_NBIT0_ERR = (
    0x0001  # 在发送报文（或应答位、主动错误标志或过载标志）期间，器件要发送显性电平（逻辑值为0的数据或标识符位），但监视的总线值为隐性。
)
CANFD_DIAGNOSTIC_FLAG_NBIT1_ERR = (
    0x0002  # 在发送报文（仲裁字段除外）期间，器件要发送隐性电平（逻辑值为1的位），但监视到的总线值为显性。
)
...
def CANFD_GetCANSpeedArg(DevHandle, pCanConfig, SpeedBpsNBT, SpeedBpsDBT):
    return USB2XXXLib.CANFD_GetCANSpeedArg(
        DevHandle, pCanConfig, SpeedBpsNBT, SpeedBpsDBT
    )
...
def CANFD_SetSchedule(DevHandle, CANIndex, pCanMsgTab, pMsgNum, pSendTimes, MsgTabNum):
    return USB2XXXLib.CANFD_SetSchedule(
...
def CANFD_StartSchedule(DevHandle, CANIndex, MsgTabIndex, TimePrecMs, OrderSend):
    return USB2XXXLib.CANFD_StartSchedule(
