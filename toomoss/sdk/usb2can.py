...
def CAN_SetSchedule(DevHandle, CANIndex, pCanMsgTab, pMsgNum, pSendTimes, MsgTabNum):
    return USB2XXXLib.CAN_SetSchedule(
        DevHandle, CANIndex, pCanMsgTab, pMsgNum, pSendTimes, MsgTabNum
    )


def CAN_StartSchedule(DevHandle, CANIndex, MsgTabIndex, TimePrecMs, OrderSend):
    return USB2XXXLib.CAN_StartSchedule(
        DevHandle, CANIndex, MsgTabIndex, TimePrecMs, OrderSend
    )
...
def CAN_BL_NodeCheck(DevHandle, CANIndex, NodeAddr, pVersion, pType, TimeOut):
    return USB2XXXLib.CAN_BL_NodeCheck(
        DevHandle, CANIndex, NodeAddr, pVersion, pType, TimeOut
    )
...
def CAN_BL_Write(DevHandle, CANIndex, NodeAddr, AddrOffset, pData, DataNum, TimeOut):
    return USB2XXXLib.CAN_BL_Write(
        DevHandle, CANIndex, NodeAddr, AddrOffset, pData, DataNum, TimeOut
    )
...
def CAN_BL_SetNewBaudRate(
    DevHandle, CANIndex, NodeAddr, pInitConfig, NewBaudRate, TimeOut
):
    return USB2XXXLib.CAN_BL_SetNewBaudRate(
        DevHandle, CANIndex, NodeAddr, pInitConfig, NewBaudRate, TimeOut
    )
