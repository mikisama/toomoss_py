...
class SPI_CONFIG(Structure):
    _fields_ = [
        (
            "Mode",
            c_ubyte,
        ),  # SPI控制方式:0-硬件控制（全双工模式）,1-硬件控制（半双工模式），2-软件控制（半双工模式）,3-单总线模式，数据线输入输出都为MOSI
...
# SPI写读数据
def SPI_WriteReadBytes(
    DevHandle, SPIIndex, pWriteData, WriteLen, pReadData, ReadLen, IntervalTimeUs
):
    return USB2XXXLib.SPI_WriteReadBytes(
        DevHandle, SPIIndex, pWriteData, WriteLen, pReadData, ReadLen, IntervalTimeUs
    )
...
def SPI_WriteReadBits(DevHandle, SPIIndex, pWriteBitStr, pReadBitStr, ReadBitsNum):
    return USB2XXXLib.SPI_WriteReadBits(
        DevHandle, SPIIndex, pWriteBitStr, pReadBitStr, ReadBitsNum
    )


# SPI 块操作相关函数
# Block模式写数据
def SPI_BlockWriteBytes(
    DevHandle, SPIIndex, pWriteData, BlockSize, BlockNum, IntervalTimeUs
):
    return USB2XXXLib.SPI_BlockWriteBytes(
        DevHandle, SPIIndex, pWriteData, BlockSize, BlockNum, IntervalTimeUs
    )


# Block模式读数据
def SPI_BlockReadBytes(
    DevHandle, SPIIndex, pReadData, BlockSize, BlockNum, IntervalTimeUs
):
    return USB2XXXLib.SPI_BlockReadBytes(
        DevHandle, SPIIndex, pReadData, BlockSize, BlockNum, IntervalTimeUs
    )


# Block模式写读数据
def SPI_BlockWriteReadBytes(
    DevHandle,
    SPIIndex,
    pWriteData,
    WriteBlockSize,
    ...
    return USB2XXXLib.SPI_BlockWriteReadBytes(
        DevHandle,
        SPIIndex,
        pWriteData,
        WriteBlockSize,
...
def SPI_SlaveWriteBytes(DevHandle, SPIIndex, pWriteData, WriteLen, TimeOutMs):
    return USB2XXXLib.SPI_SlaveWriteBytes(
        DevHandle, SPIIndex, pWriteData, WriteLen, TimeOutMs
    )


# 从机模式读数据
def SPI_SlaveReadBytes(DevHandle, SPIIndex, pReadData, ReadLen, TimeOutMs):
    return USB2XXXLib.SPI_SlaveReadBytes(
        DevHandle, SPIIndex, pReadData, ReadLen, TimeOutMs
    )


# SPI从机模式下读写数据
def SPI_SlaveReadWriteBytes(
    DevHandle, SPIIndex, pReadData, ReadDataLen, pWriteData, WriteDataLen, TimeOutMs
):
    return USB2XXXLib.SPI_SlaveReadWriteBytes(
        DevHandle, SPIIndex, pReadData, ReadDataLen, pWriteData, WriteDataLen, TimeOutMs
    )


# SPI从机模式下写读数据
def SPI_SlaveWriteReadBytes(
    DevHandle, SPIIndex, pWriteData, WriteDataLen, pReadData, ReadDataLen, TimeOutMs
):
    return USB2XXXLib.SPI_SlaveWriteReadBytes(
        DevHandle, SPIIndex, pWriteData, WriteDataLen, pReadData, ReadDataLen, TimeOutMs
    )
...
# SPI从机模式下连续发送数据
def SPI_SlaveContinueWrite(DevHandle, SPIIndex, pWriteData, WriteLen):
    return USB2XXXLib.SPI_SlaveContinueWrite(DevHandle, SPIIndex, pWriteData, WriteLen)
...
def SPI_FlashWrite(DevHandle, SPIIndex, StartAddr, pWriteData, WriteLen):
    return USB2XXXLib.SPI_FlashWrite(
        DevHandle, SPIIndex, StartAddr, pWriteData, WriteLen
    )
...
def SPI_FlashReadFast(DevHandle, SPIIndex, StartAddr, pReadData, ReadLen):
    return USB2XXXLib.SPI_FlashReadFast(
        DevHandle, SPIIndex, StartAddr, pReadData, ReadLen
    )
